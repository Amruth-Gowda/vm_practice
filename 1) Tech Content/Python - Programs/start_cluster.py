...
def Parameter_Count():
	if len(sys.argv) == 8:
			activity_cmd = start_cluster(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7])
			return activity_cmd
	else:
		print("invalid parameters count")
		return "invalid command count"

def start_cluster(host, username, cluster_name, password, port):
	actual_cmd = "timeout 10s cmps -H %s --user %s -c %s --password %s --port %s  -e \ 'use %s;start_cluster cluster\'" %(host, username, cluster_name, password, port, cluster_name)
	return actual_cmd
