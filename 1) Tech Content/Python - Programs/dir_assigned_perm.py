...
#	client_stdin, client_stdout, client_stderr = client.exec_command("sudo cat /tmp/xxxx")
	...
	#print output
	err_output = err_output[0].decode('utf-8') # converting list to unicode
	#print(output)
	#print(err_output)
...
def Parameter_Count():
	if len(sys.argv) == 7:
			activity_cmd = dir_assigned_perm(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
			return activity_cmd
	else:
		print("invalid parameters count")
		return "invalid command count"

def dir_assigned_perm(hostname, port, dir_name, perm):
	actual_cmd = "timeout 10s curl -i -X PUT \"http://%s:%s/webhdfs/v1/%s?op=MKDIRS&user.name=hdfs&permission=%s\"" %(hostname, port, dir_name, perm)
	return actual_cmd
...
def MainFunc():
	...
	try:
			...
			print(get_time()+"\tDirectory created successfully!")
			file_close()
