...
def Parameter_Count():
	if len(sys.argv) == 8:
		activity_cmd = cm_deploy(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7])
		return activity_cmd
	else:
		return "invalid command count"

def cm_deploy(username, password, host, port, path):
	actual_cmd = "curl --max-time 2 -s -u \'%s:%s\' \"http://%s:%s/api/v10/cm/deployment\" >  %s/cm-deployment.json" %(username, password, host, port, path)
	return actual_cmd
