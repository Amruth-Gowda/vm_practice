...
def Parameter_Count():
	if len(sys.argv) == 8:
		activity_cmd = stop_hive(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7])
		return activity_cmd
	else:
		return "invalid command count"

def stop_hive(host, username, cluster_name, password, port):
	actual_cmd = "timeout 10s cmps -H %s -p %s -u %s -c %s --password %s  -e \'use %s;stop_service hive\'" %(host, port, username, cluster_name, password, cluster_name)
	return actual_cmd
