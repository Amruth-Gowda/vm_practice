...
def Parameter_Count():
	if len(sys.argv) == 4:
			activity_cmd = hdfs_balancer(sys.argv[3])
			return activity_cmd
	else:
		print("invalid parameters count")
		return "invalid command count"

def hdfs_balancer(thresh):
	actual_cmd = "sudo -u hdfs hdfs balancer -threshold %s" %thresh
	return actual_cmd
...
def MainFunc():
	...
	try:
			...
			print(get_time()+"\tThe cluster is balanced.")
			file_close()
