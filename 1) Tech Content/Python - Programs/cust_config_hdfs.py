...
def Parameter_Count():
	if len(sys.argv) == 9:
		activity_cmd = cust_config_hdfs(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8])
		return activity_cmd
	else:
		return "invalid command count"

def cust_config_hdfs(username, password, host, port, cluster_name, service_name):
	actual_cmd = "curl --max-time 2 -s -u %s:%s \'http://%s:%s/api/v1/clusters/%s/services/%s/config\'" %(username, password, host, port, cluster_name, service_name)
	return actual_cmd
