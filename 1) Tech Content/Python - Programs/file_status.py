...
def Parameter_Count():
	if len(sys.argv) == 6:
			activity_cmd = file_status(sys.argv[3], sys.argv[4], sys.argv[5])
			return activity_cmd
	else:
		print("invalid parameters count")
		return "invalid command count"

def file_status(hostname, port, path):
	actual_cmd = "timeout 10s curl -i \"http://%s:%s/webhdfs/v1/%s?op=GETFILESTATUS\"" %(hostname, port, path)
	return actual_cmd
...
def MainFunc():
	...
	try:
			...
			#print(get_time()+"\tHDFS metadata backup created successfully!")
