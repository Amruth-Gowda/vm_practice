...
def Parameter_Count():
	if len(sys.argv) == 6:
			activity_cmd = dir_default_perm(sys.argv[3], sys.argv[4], sys.argv[5])
			return activity_cmd
	else:
		print("invalid parameters count")
		return "invalid command count"

def dir_default_perm(hostname, port, dir_name):
	actual_cmd = "timeout 10s curl -i -X PUT \"http://%s:%s/webhdfs/v1/%s?op=MKDIRS&user.name=hdfs\"" %(hostname, port, dir_name)
	return actual_cmd
