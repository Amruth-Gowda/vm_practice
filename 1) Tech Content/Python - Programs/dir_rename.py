...
def validate_arguments_count(user_input):
        ...
        print("\tArguments check in  progress....")
        if  input_count != 4:
...
#	client_stdin, client_stdout, client_stderr = client.exec_command("sudo cat /tmp/xxxx")
	...
	# err_output = err_output[1].decode('utf-8') # converting list to unicode
	#print(output)
	#print(output1)
	#print(err_output)
	#print(err_output1)
...
def Parameter_Count():
	if len(sys.argv) == 7:
			activity_cmd = dir_rename(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
			return activity_cmd
	else:
		print("invalid parameters count")
		return "invalid command count"

def dir_rename(hostname, port, dir_name, new_name):
	actual_cmd = "timeout 10s curl -i -X PUT \"http://%s:%s/webhdfs/v1/%s?op=RENAME&user.name=hdfs&destination=/%s\"" %(hostname, port, dir_name, new_name)
	return actual_cmd
...
def MainFunc():
	...
	try:
			...
			print(get_time()+"\tDirectory renamed successfully!")
			file_close()
