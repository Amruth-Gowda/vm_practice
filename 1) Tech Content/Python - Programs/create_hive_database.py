...
def Parameter_Count():
        if len(sys.argv) == 6:
                activity_cmd = create_hive_database(sys.argv[3], sys.argv[4], sys.argv[5])
                return activity_cmd
        else:
                return "invalid command count"

def create_hive_database(host, port, dbname):
	actual_cmd = "curl -s -X PUT -HContent-type:application/json -d '{ \"comment\":\"Hello there\", \"location\":\"/user/hive/warehouse/\", \"properties\":{\"a\":\"b\"}}' 'http://%s:%s/templeton/v1/ddl/database/%s?user.name=hive'" % (host, port, dbname)
	print(actual_cmd)
	return actual_cmd
