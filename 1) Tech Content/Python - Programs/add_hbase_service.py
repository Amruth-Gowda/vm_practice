...
def validate_arguments_count(user_input):
        ...
        if  input_count != 4:
                ...
                print("Improper input!!!!!!!!! \tRefer the instructions and enter the input accordingly")
                return_value = 0
        ...
        if ping_check == 1:
                ...
                if input_cloud == 'aws':
                        ...
                        if input_type == 'key':
                                ...
                                print("\tEnter allowed type of Authentication \n\tRefer banner for more information")
                                return 0
                else:
                        file_write("Enter only allowed service provider name \n\tRefer banner for more information")
                        print("\tEnter only allowed service provider name \n\tRefer banner for more information")
                        return 0
        else:
...
def Parameter_Count():
	if len(sys.argv) == 10:
		activity_cmd = add_hbase_service(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9])
		return activity_cmd
	else:
		return "invalid command count"

def add_hbase_service(username, password, service_name, service_type, host, port, cluster_name):
	actual_cmd = "curl --max-time 2 -s -X POST -H \"Content-Type:application/json\" -u %s:%s -d '{ \"items\": [ { \"name\": \"%s\", \"type\": \"%s\" } ] }\' \'http://%s:%s/api/v1/clusters/%s/services\'" %(username, password, service_name, service_type, host, port, cluster_name)
	return actual_cmd
