...
def Parameter_Count():
        if len(sys.argv) == 6:
                activity_cmd = delete_hive_database(sys.argv[3], sys.argv[4], sys.argv[5])
                return activity_cmd
        else:
                return "invalid command count"

def delete_hive_database(host, port, dbname):
        actual_cmd = "curl -s -X DELETE 'http://%s:%s/templeton/v1/ddl/database/%s?user.name=hive'" % (host, port, dbname)
        return actual_cmd
