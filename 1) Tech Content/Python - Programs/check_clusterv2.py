...
def execute_command(cmd_recieved):
	...
	#client_stdin, client_stdout, client_stderr = client.exec_command("echo $?")
#	client_stdin, client_stdout, client_stderr = client.exec_command("sudo cat /tmp/xxxx")
	...
	#err_output = err_output[0].decode('utf-8')
	#err_output = err_output.encode('ascii','ignore')
	print(output)
	#print(err_output)
...
def check_clusters(username, password, host, port):
	actual_cmd = "curl --max-time 2 -s -u %s:%s \'http://%s:%s/api/v1/clusters\' | egrep \'name\'" %(username, password, host, port)
	return actual_cmd
...
def MainFunc():
	...
	try:
			...
			print(get_time()+"\tActivity Execution Completed")
			file_close()
