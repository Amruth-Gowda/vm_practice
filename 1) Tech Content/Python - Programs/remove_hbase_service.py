...
def Parameter_Count():
	if len(sys.argv) == 10:
		activity_cmd = remove_hbase_service(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7], sys.argv[8], sys.argv[9])
		return activity_cmd
	else:
		return "invalid command count"

def remove_hbase_service(username, password, service_name, service_type, host, port, cluster_name):
	actual_cmd = "curl --max-time 2 -s -X DELETE -H \"Content-Type:application/json\" -u %s:%s -d '{ \"items\": [ { \"name\": \"%s\", \"type\": \"%s\" } ] }\' \'http://%s:%s/api/v1/clusters/%s/services/%s\'" %(username, password, service_name, service_type, host, port, cluster_name, service_name)
	return actual_cmd
