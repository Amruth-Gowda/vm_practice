...
def Parameter_Count():
	if len(sys.argv) == 5:
			activity_cmd = load_data_hdfs(sys.argv[3], sys.argv[4])
			return activity_cmd
	else:
		print("invalid parameters count")
		return "invalid command count"

def load_data_hdfs(src_path, dest_path):
	actual_cmd = "hdfs dfs -put %s %s" %(src_path, dest_path)
	return actual_cmd
...
def MainFunc():
	...
	try:
			...
			print(get_time()+"\tData loaded into HDFS successfully")
			file_close()
