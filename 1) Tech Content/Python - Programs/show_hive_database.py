...
def Parameter_Count():
        if len(sys.argv) == 5:
                activity_cmd = show_hive_database(sys.argv[3], sys.argv[4])
                return activity_cmd
        else:
                return "invalid command count"

def show_hive_database(host, port):
	actual_cmd = "curl -s 'http://%s:%s/templeton/v1/ddl/database?user.name=hive=showdatabases*'" % (host, port)
	return actual_cmd
