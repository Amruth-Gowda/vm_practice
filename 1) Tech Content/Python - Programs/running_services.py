...
#	client_stdin, client_stdout, client_stderr = client.exec_command("sudo cat /tmp/xxxx")
	...
	#if any(err_output):
	if (output == "  \"items\" : [ {\n"):
	#if "[u'{\n', u'  \"message\" : \"Cluster \'Cluster \' not found.\"\n', u'}']" in output1:
...
def Parameter_Count():
	if len(sys.argv) == 8:
		activity_cmd = running_services(sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7])
		return activity_cmd
	else:
		return "invalid command count"

def running_services (username, password, host, port, cluster_name):
	actual_cmd = "curl --max-time 2 -s -u %s:%s  'http://%s:%s/api/v1/clusters/%s/services'" %(username, password, host, port, cluster_name)
	return actual_cmd
